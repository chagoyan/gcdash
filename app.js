...
function collectSettings() {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    if (key.startsWith('course-color-')) courseColors[key.replace('course-color-', '')] = localStorage.getItem(key);
    if (key.startsWith('course-order-')) courseOrder[key.replace('course-order-', '')] = JSON.parse(localStorage.getItem(key));
  }
