...
function toggleOwned() {
  ...
  document.getElementById('owned-toggle').textContent = showOwnedOnly ? 'Showing: My courses' : 'Showing: All courses';
  renderCourses();
}
...
function renderCourses() {
  ...
  filtered.forEach(c => { const s = c.courseState || 'OTHER'; (groups[s] || groups.OTHER).push(c); });
  ...
  if (!tabDefs.find(t => t.key === activeTab) && tabDefs.length) activeTab = tabDefs[0].key;
...
function switchTab(key) {
  ...
  document.querySelectorAll('.tab-panel').forEach(el => el.classList.toggle('active', el.id === 'tab-panel-' + key));
}
...
function showDetail(courseId) {
  ...
  if (merged.length) {
    ...
    document.getElementById('detail-body').innerHTML = `<div class="empty-note">No assignments or materials found.</div>`;
  }
...
async function monitorSelected() {
  ...
  try {
    ...
    const studentMap = {};
    students.forEach(s => {
      studentMap[s.userId] = { name: s.profile.name.fullName, userId: s.userId, earned: 0, possible: 0, missing: 0 };
    });
...
function showProgressReport(studentMap, courseId) {
  ...
  let html = `
    ...
    <table class="progress-table">
      <thead><tr>
        <th>Student</th><th>Grade</th><th>Earned / Possible</th><th>Missing</th>
      </tr></thead>
      <tbody>`;
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Topic block — bordered container per week */
  .topic-block {
  ...
  /* Assignment item — individual bordered box */
  ul { list-style: none; padding-left: 0; }
  li.assignment-item {
    border: 1px solid #e0e0e0;
    border-radius: 8px;
    padding: ${isSummary ? '6px 12px' : '10px 14px'};
    margin-bottom: 6px;
    page-break-inside: avoid;
    background: #fafafa;
  }
...
async function downloadAllMd() {
  ...
  a.href = url; a.download = 'classroom-courses.zip'; a.click();
