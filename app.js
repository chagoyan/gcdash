...
function connectGoogle() {
	...
	if (!clientId) {
		setStatus(
			'auth-status',
			'Client ID not found. Please check your config.js or environment variable.',
			'error',
		);
		return;
	}
	if (!window.google || !window.google.accounts) {
		setStatus(
			'auth-status',
			'Google Identity Services not ready. Please refresh and try again.',
			'error',
		);
		return;
	}
